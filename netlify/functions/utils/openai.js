...
【鑑定結果の形式】
- 鑑定結果は詳しく、丁寧に、長文で記載してください（800文字以上を目安に）
- 見出しや強調には必ず【】を使用してください（例：【現状】【未来の展望】【具体的なアドバイス】）
- 段落ごとに改行を入れて読みやすくしてください
- 過去・現在・未来の流れを詳しく解説してください
- 具体的なアドバイスは3つ以上、詳しく記載してください
- 最後に温かい励ましのメッセージを添えてください
...
- - リスト ← 絶対に使用禁止
- 1. 番号リスト ← 絶対に使用禁止
これらのMarkdown記法は絶対に使わず、必ず【】で見出しを表現してください。
...
【返答の形式】
- 返答は丁寧に、長めに記載してください（500文字以上を目安に）
- 見出しや強調には必ず【】を使用してください
- 段落ごとに改行を入れて読みやすくしてください
- 温かく寄り添う言葉遣いを心がけてください
...
- ## 見出し ← 絶対に使用禁止
- - リスト ← 絶対に使用禁止
これらのMarkdown記法は絶対に使わず、必ず【】で表現してください。
...
` : ''
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: messages,
      temperature: 0.8,
      max_tokens: 2000, // より長い鑑定結果のために増量
...
` : ''
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: messages,
      temperature: 0.7,
      max_tokens: 1500, // より詳しい相談応答のために増量
